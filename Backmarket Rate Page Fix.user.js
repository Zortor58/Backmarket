...
var pageSav = document.querySelector('.merchants-savs-sav');
...
if(pageRate != null) {
	node.href = "?country=&lower=&rate_and_desc=rate_and_desc&upper=&page="+page;

	if(page > 2) {
  	page = page - 2;
  	nodeBack.href = "?country=&lower=&rate_and_desc=rate_and_desc&upper=&page="+page;
	}
}

if(pageRate && window.location.href == "https://www.backmarket.fr/bo_merchant/rates/"){
	window.location.href = "https://www.backmarket.fr/bo_merchant/rates/?country=&lower=&rate_and_desc=rate_and_desc&upper=";
}
...
if(pageListing) {
	var dangerItems = document.getElementsByClassName("danger");
	var arr = Array.prototype.slice.call( dangerItems );
	arr.forEach(function(element) {
		priceSuggest = element.cells[4].childNodes[8].textContent;
  	priceSuggest = priceSuggest.replace(/\n/g, '');
  	priceSuggest = priceSuggest.replace(/\s+/g, '');
 		element.cells[4].childNodes[11].value = priceSuggest;
	});
}
...
{
  var savItems = document.getElementsByClassName("sav_table_row");
 	var arr2 = Array.prototype.slice.call( savItems );   
